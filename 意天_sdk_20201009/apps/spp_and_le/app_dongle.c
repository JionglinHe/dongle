...
static const u16 mouse_notify_handle[4] = {0x0025, 0x0047, 0x003b, 0x003f};
//[0] 键盘; [1]鼠标; [3]xx
/* static u8 get_report_map[256]; */
/* static u8 get_report_map_size = 0; */


extern u32 hid_send_consumer_control(u32 key);
extern u32 hid_send_keyboard_data(struct keyboard_data_t *p);
extern hid_send_mouse_data(struct _mouse_point_t *p);
extern u32 custom_hid_send_data(u8 *buffer);

static void ble_2_hid(att_data_report_t *report_data)
{
    u8 buf[64]={0};
    if (report_data->value_handle == mouse_notify_handle[0])
    {
        hid_send_keyboard_data(report_data->blob);
    }
    else if(report_data->value_handle == mouse_notify_handle[1])
    {
        hid_send_mouse_data(report_data->blob);
    }
    else if(report_data->value_handle == mouse_notify_handle[2])
    {
        memcpy(buf, report_data->blob, report_data->blob_length );
        custom_hid_send_data(buf);
    }
    else if(report_data->value_handle == mouse_notify_handle[3])
    {
        u32 dat =0;
        dat += report_data->blob[0];
        dat += (report_data->blob[1]<<8);
        dat += (report_data->blob[2]<<16);
        hid_send_consumer_control(  dat );
...
//static const u8 dongle_remoter_name1[] = "XFTR201";//
static const u8 dongle_remoter_name2[] = "XFTR201";//
//static const u8 dongle_remoter_macaddr1[] = {0X66, 0X0D, 0X00, 0XEE, 0XF3, 0X0C};//
static const u8 dongle_remoter_macaddr1[] = {0X99, 0X0D, 0X00, 0XEE, 0XF3, 0X0C};//
...
//};
static const client_match_cfg_t match_dev01 = {
    .create_conn_mode = BIT(CLI_CREAT_BY_ADDRESS),             //连接从机设备的方式:有地址连接,设备名连接,厂家标识连接
    .compare_data_len = sizeof(dongle_remoter_macaddr1) ,   //(连接内容长度)
    .compare_data = dongle_remoter_macaddr1,                   //根据连接方式,填内容
    .bonding_flag = 1,//绑定                              //是否配对,如果配对的话, 下次就会直接连接
};
...
static void dongle_enable_notify_ccc(void)
{
    ...
    ble_client_api->opt_comm_send(mouse_notify_handle[3] + 1, &mouse_ccc_value, 2, ATT_OP_WRITE);
}
...
static void bt_function_select_init()
{
    ...
    {
        u8 tmp_ble_addr[6]={0xBD,0X0C,0X11,0XEE,0XF3,0X0C};
